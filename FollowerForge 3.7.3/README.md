# FollowerForge 3.7.3 - Windows tool (not a Skyrim mod)
...
## FaceGen warning in 3.7.3

If a build says the exported head was not copied, the plugin is still valid and the sculpt was left out on purpose. The message names the Creation Kit steps (open the plugin, select the actor, Ctrl+F4) and where `ck-handoff-report.json` was written. Ctrl+F4 bakes the sliders and tint layers stored on the actor. It does not include a RaceMenu sculpt.

A head that already points at a CharGen or facetint texture is retargeted on the tint slot (the 7th texture line). The specular map on the next line is left alone.

## Community fixes in 3.7.2

The "How she regards you" picker wrote a different relationship rank than the one you picked (choosing Ally could record Friend). Preset head parts RaceMenu could not resolve — usually hair — were silently dropped, so a follower could build successfully and still show up bald. Both are fixed, and the build report now warns when an appearance has tint but no complexion set, which is the usual cause of a face that does not match the body.

## Reliability fixes in 3.7.1

MO2 conflict winners now follow the saved priority order. Missing selected head meshes fail clearly. Custom output paths cannot bypass game-folder protection, and builds can publish across drives without deleting the old output before preparation finishes.
