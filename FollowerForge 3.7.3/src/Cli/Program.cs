...
public static class Program
{
    ...
    private static int CmdBuild(Dictionary<string, string> opts)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"FaceGen  : {FaceGenPlayerText.Summary(result.Manifest.HasFaceGen, result.Validation.Findings)}");
    ...
    private static int CmdRaces(Dictionary<string, string> opts)
    {
        ...
        // --creatures shows what the combat-transformation picker offers, which is wider
        // than the identity picker on purpose: she cannot BE a dragon, but she can turn
        // into one, and the face belongs to her base race either way.
        var creatures = opts.ContainsKey("creatures");
        var offered = SkyrimRecords.RaceSuitability.Offer(all, includeCreatures: creatures);
        var limit = opts.TryGetValue("limit", out var lt) && int.TryParse(lt, out var l) ? l : 25;
        var scope = creatures ? "offered (creatures included)" : "usable";

        Console.WriteLine($"{all.Count} race records → {offered.Count} {scope}\n");
    ...
    private static int CmdIndex(Dictionary<string, string> opts)
    {
        ...
        // A corrupt cache must not brick the very command that rebuilds it. The wizard already
        // quarantines and retries; the CLI used to throw SqliteException and leave the user stuck.
        try
        {
        ...
        {
            Console.WriteLine("Catalogue cache is damaged — rebuilding it from scratch.");
            CatalogDb.QuarantineBrokenCache(dbPath, Log.Logger);
        }

        CatalogBuilder.CatalogSummary summary;
        try
        {
            summary = new CatalogBuilder(Log.Logger).Build(env, dbPath);
        }
        catch (Exception ex) when (CatalogDb.IsCacheFailure(ex))
        {
            Console.WriteLine("Catalogue cache is damaged — rebuilding it from scratch.");
