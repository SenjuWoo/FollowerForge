...
public sealed class FollowerBuilder(ILogger log)
{
    ...
    /// </param>
    /// <param name="publishRoot">
    /// Optional user-chosen parent for the published follower folder. When set, the plugin
    /// lands in <c>publishRoot\\Name</c> (so a Vortex/MO2 mods folder becomes one installable
    /// mod per follower) and that explicit destination is allowed even if it sits under a
    /// manager staging root. Staging still uses <paramref name="workspaceRoot"/>.
    /// </param>
        ...
        string? publishRoot = null)
    {
        ...
        var finalParent = string.IsNullOrWhiteSpace(publishRoot)
            ? Path.Combine(workspaceRoot, "builds")
            : Path.GetFullPath(publishRoot.Trim().Trim('"'));
        if (!string.IsNullOrWhiteSpace(publishRoot))
        {
            // An explicit mods destination must never exempt the game or saves.
            var hardGuard = new WriteGuard();
            hardGuard.Protect(env.GameRootPath);
            hardGuard.Protect(env.GameDataPath);
            hardGuard.Protect(Path.Combine(
                Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments),
                "My Games", "Skyrim Special Edition"));
            hardGuard.EnsureWritable(finalParent);
            guard.Allow(finalParent);
        }
        guard.EnsureWritable(finalParent);
        ...
        var finalDir = Path.Combine(finalParent, SafeName(profile.Name));

        // Clean staging (idempotent, deterministic).
        if (Directory.Exists(staging)) Directory.Delete(staging, recursive: true);
        Directory.CreateDirectory(staging);

        // Materialise the MO2 hardlink plugin view once so every open-by-filename path
        // (records, cells, master expansion) sees mod-only masters such as SOSVoices.esm.
        // Vortex keeps PluginReadRoot == GameDataPath; no view work.
        var pluginReadRoot = EnsurePluginReadRoot(env, log);
        ...
        using var resolver = new RecordResolver(pluginReadRoot, log);
        ...
        // lighting/flags instead of blanking them. Library locations and manual Placement.Cell
        // both need this — previously manual Cell was ignored and cellSource stayed null.
        using var cellSources = new CellSourceResolver(pluginReadRoot, log);
        ICellGetter? cellSource = null;
        if (location is not null)
        {
            cellSource = cellSources.Resolve(Mutagen.Bethesda.Plugins.FormKey.Factory(location.CellFormKey));
            if (cellSource is null)
        ...
        else if (profile.Placement.Cell is { } manualCell)
        {
            cellSource = cellSources.Resolve(Mutagen.Bethesda.Plugins.FormKey.Factory(manualCell.FormKey));
            if (cellSource is null)
                report.Add(ValidationSeverity.Warning, "LOCATION_CELL_UNREADABLE",
                    $"Could not read manual placement cell {manualCell.FormKey}; " +
                    "the placement override may not carry the room's lighting.");
        }
        ...
        // Master chain expansion must open real TES4 headers. Under MO2 those live in the
        // hardlink plugin view (and underlying mod folders), never only in Steam Data.
        if (env.ActiveProfileId is not null && Directory.Exists(pluginReadRoot))
        {
        ...
        ValidationReport report)
    {
        ...
        {
            var export = FindCharGenExport(appearance.CharGenExportName, env);
            presetJslot = export?.JslotPath;
        ...
        if (presetJslot is not null)
            ReportPresetBodyShape(
                new CharGenDiscovery(log).ReadJslotBodyMorphCount(presetJslot), report);
        ...
        // Surfaced on the FINAL appearance, so the warning states what the NPC record will
        // actually carry — whether the gap came from an old preset or from an unresolved ref.
        if (appearance.UnresolvedHeadParts > 0)
            report.Add(ValidationSeverity.Warning, "PRESET_HEADPART_UNRESOLVED",
                $"{appearance.UnresolvedHeadParts} head part(s) in the preset could not be resolved to a record — "
                + "her hair/eyes/brows may be missing in game. Enable the mod that provides them and "
                + "re-save the preset with RaceMenu.");
        if (appearance.TintLayers.Count > 0 && appearance.HeadTextureSet is null)
            report.Add(ValidationSeverity.Warning, "PRESET_NO_HEAD_TEXTURE",
                "The appearance carries tint layers but no head texture set (complexion) — her face will "
                + "use her race's default complexion while the tint carries the preset's colors, so face "
                + "and body tones can disagree. Re-save the preset with RaceMenu so it records the "
                + "complexion it was built on, and install that complexion mod.");
    ...
    /// </summary>
    /// <summary>
    /// Says what pinning a body costs. Leaving it null is the normal case: she then wears whatever
    /// body the player installed, which is what OBody needs in order to reshape her at all.
    /// </summary>
    internal static void ReportPinnedBody(RecordRef? skin, ValidationReport report)
    {
        if (skin is null) return;

        report.Add(ValidationSeverity.Info, "BODY_PINNED",
            $"Her body is pinned to skin record {skin.FormKey}, so the mod that defines it becomes "
            + "a hard requirement for anyone who installs her, and OBody will no longer reshape "
            + "her. Leave the body empty to use whatever body the player already has.");
    }
    ...
    private CharGenExport? FindCharGenExport(string name, EnvironmentSnapshot env) =>
        new CharGenDiscovery(log).Discover(env)
    ...
    ///
    /// A werewolf needs nothing installed beyond Skyrim — only WerewolfBeastRace. The vanilla
    /// beast-form visual spell is not used; it re-applies the wolf race after combat. A custom
    /// transformation points at the user's own records, so those mods become requirements.
    /// </summary>
        ...
        string staging, ValidationReport report)
    {
        ...
        var what = transformation.Kind == TransformKind.Werewolf
            ? "a werewolf, using the game's own beast race"
            : "her chosen form";
        ...
        string staging, ValidationReport report, CatalogDb? catalog)
    {
        ...
        if (nifPath is null)
        {
            if (!string.IsNullOrWhiteSpace(app.CharGenNifPath))
                report.Add(ValidationSeverity.Error, "FACE_NIF_MISSING",
                    $"The selected head mesh is missing: {app.CharGenNifPath}. Restore it or clear the face selection.");
            return null;
        }

        FaceGen.FaceGenSwapper.TextureResolver resolver =
            rel => ResolveFaceTexture(rel, env, catalog);
        ...
        // NeedsCreationKit is already a warning on result.Findings, with the steps in the text.
        report.AddRange(result.Findings);
        return new FaceGenOutcome(result, requiredPlugins);
    }

    /// <summary>
    /// Resolves a NIF texture against the catalogue first, then MO2 mods / deployed Data.
    /// The fallback matters for valid Vortex hardlinks absent from a stale/incomplete manifest,
    /// and for MO2 loose files that never appear in Steam Data.
    /// It is read-only and rejects paths that escape Data under Vortex.
    /// </summary>
    public static (bool Resolved, string? Container) ResolveFaceTexture(
        string relPath, EnvironmentSnapshot env, CatalogDb? catalog)
    {
        ...
        {
            var mo2Path = Mo2PathResolver.ResolveRelative(env, normalized);
            if (mo2Path is not null) return (true, "MO2 mod file");
        }

        var dataRoot = Path.GetFullPath(env.GameDataPath)
    ...
    /// <summary>Back-compat overload for tests that only supply a single Data root.</summary>
    public static (bool Resolved, string? Container) ResolveFaceTexture(
        string relPath, string gameDataPath, CatalogDb? catalog) =>
        ResolveFaceTexture(relPath, new EnvironmentSnapshot
        {
            Manager = ModManagerKind.Vortex,
            ManagerLabel = "Vortex",
            GameRootPath = Path.GetDirectoryName(gameDataPath) ?? gameDataPath,
            GameDataPath = gameDataPath,
            PluginDataPath = gameDataPath,
            InstancePath = gameDataPath,
            StagingPath = gameDataPath,
            ProfilesPath = gameDataPath,
            RuntimePluginsTxtPath = gameDataPath,
        }, catalog);
        ...
        AppearanceSpec app, EnvironmentSnapshot env, CatalogDb? catalog)
    {
        ...
        var export = FindCharGenExport(app.CharGenExportName, env);
        if (export is null)
        {
        ...
        FollowerProfile profile, FollowerManifest manifest, ValidationReport report)
    {
        ...
        return $$"""
            ...
            <table class="kv">
              ...
              <tr><td>FaceGen</td><td>{{FaceGenPlayerText.Summary(manifest.HasFaceGen, report.Findings)}}</td></tr>
            </table>
            <h2>Validation</h2>
    ...
    [
        ...
        IndexedRecordType.Ammo,
    ];

    /// <summary>
    /// Skyrim stores a non-localized plugin's strings in ONE single-byte codepage (Windows-1252
    /// for a Western install), so Cyrillic, Greek and CJK cannot survive: Mutagen writes '?' for
    /// each. Western accents (Ëlïna, Ökÿ) encode fine and are left alone. Writing a different
    /// codepage would only shift the breakage to everyone whose game is not that language, so the
    /// build says what will be lost instead of silently mangling her name.
    /// </summary>
    internal static void ReportUnencodableName(string name, ValidationReport report)
    {
        var lost = name.Where(c => c > 0xFF).Distinct().ToList();
        if (lost.Count == 0) return;

        report.Add(ValidationSeverity.Warning, "NAME_CHARACTERS_LOST",
            $"Her name contains {lost.Count} character(s) Skyrim cannot store in a plugin "
            + $"({string.Join(" ", lost)}) — the game will show '?' for each. Western European "
            + "accents (é ü ñ ö) are fine; Cyrillic, Greek and CJK are not, because a plugin holds "
            + "one single-byte codepage. Rename her using Latin characters to avoid this.");
    }

    /// <summary>
    /// A RaceMenu preset can carry a whole body shape in its "bodyMorphs" block — OBody and
    /// BodySlide write their sliders there. An NPC record has nowhere to put that: it stores head
    /// data, not a body. So the shape is lost on export, and until now nothing said so.
    /// The fix on the player's side is a BodySlide/OBody preset for her, not a plugin field.
    /// </summary>
    internal static void ReportPresetBodyShape(int shapedSliders, ValidationReport report)
    {
        if (shapedSliders <= 0) return;

        report.Add(ValidationSeverity.Warning, "PRESET_BODY_NOT_TRANSFERRED",
            $"The RaceMenu preset shapes {shapedSliders} body slider(s), and a plugin cannot "
            + "store a body — an NPC record holds head data only. Her face comes across exactly; "
            + "her body will be whatever the player has installed. To give her that exact shape, "
            + "save it as a BodySlide/OBody preset for her instead. RaceMenu overlays (tattoos, "
            + "warpaint and scars applied as overlay layers) do not transfer either.");
    }

    /// <summary>
    /// Both a legacy outfit and hand-picked armor is a supported combination, and the outfit
    /// wins: Skyrim decides what an NPC initially wears from DefaultOutfit, so the picked pieces
    /// stay in her pack rather than on her body. The two live on different pages and look like
    /// they combine, so say plainly which one the game will honour.
    /// </summary>
    internal static void ReportOutfitOverridesArmor(
        FollowerProfile profile, RecordRef outfit, CatalogDb? catalog, ValidationReport report)
    {
        if (profile.EquippedArmor.Count == 0) return;

        var record = catalog?.GetRecord(outfit.FormKey);
        var name = record is null
            ? outfit.FormKey
            : !string.IsNullOrWhiteSpace(record.DisplayName) ? record.DisplayName!
            : !string.IsNullOrWhiteSpace(record.EditorId) ? record.EditorId!
            : outfit.FormKey;

        report.Add(ValidationSeverity.Warning, "OUTFIT_OVERRIDES_ARMOR",
            $"She will start in '{name}', not the {profile.EquippedArmor.Count} armor piece(s) "
            + "you picked — an outfit decides what an NPC initially wears. Those pieces are "
            + "still in her inventory, so you can trade them to her in game. Clear the outfit if "
            + "you want her wearing your own selection instead.");
    }
        ...
        FollowerProfile profile, CatalogDb? catalog, ValidationReport report)
    {
        ...
        if (profile.Outfit is { } outfit)
        {
            ...
            ReportOutfitOverridesArmor(profile, outfit, catalog, report);
        }
        if (profile.SkinArmor is { } skin)
            Check(skin, "body/skin armor", [IndexedRecordType.Armor]);
        ReportPinnedBody(profile.SkinArmor, report);
        ReportUnencodableName(profile.Name, report);
        ...
        foreach (var ammo in profile.Ammo)
            Check(ammo, "arrows/bolts", [IndexedRecordType.Ammo]);
        ...
        ValidationReport report)
    {
        ...
        {
            if (Mo2PathResolver.PluginIsInstalled(env, master)) continue;
            var where = env.Manager == ModManagerKind.Mo2
                ? "the active MO2 profile (mods/overwrite) or game Data"
                : "Skyrim Data";
            report.Add(ValidationSeverity.Error, "MASTER_FILE_MISSING",
                $"Generated plugin requires {master}, but it is not installed in {where}.");
        }
    }

    /// <summary>
    /// Ensures MO2's FollowerForge hardlink view contains every enabled load-order plugin, then
    /// returns the directory that open-by-filename APIs must use. Vortex: game Data.
    /// </summary>
    internal static string EnsurePluginReadRoot(EnvironmentSnapshot env, ILogger log)
    {
        if (env.Manager == ModManagerKind.Mo2 && env.ActiveProfileId is not null)
        {
            try
            {
                var (entries, _) = new LoadOrderBuilder(log).BuildEntryList(env);
            ...
            {
                log.Warning(ex,
                    "Could not materialise MO2 plugin view; master expansion may fail for mod-only masters.");
            }
        }
        return env.PluginReadRoot;
    }
        ...
        FollowerProfile profile, CatalogDb? catalog, ValidationReport report)
    {
        ...
        // The outfit-plus-armor case is reported once, by ReportOutfitOverridesArmor, which
        // names the outfit and states outright which one the engine honours. The warning
        // that used to live here said "may choose" about something that is not in doubt.
