...
public sealed class CharGenDiscovery(ILogger log)
{
    ...
    {
        // First win via TryAdd. Enumerate highest-priority roots first.
        // MO2: overwrite/mods (highest first) then game Data (lowest).
        // Vortex: game Data only (deployed hardlinks already resolve winners).
    ...
    private IReadOnlyList<CharGenExport> DiscoverInDataRoot(string gameDataPath)
    {
        ...
        foreach (var nif in Directory.EnumerateFiles(dir, "*.nif", SearchOption.TopDirectoryOnly))
        {
            ...
            exports.Add(new CharGenExport
            {
                ...
                HeadShapeCount = CountShapes(nif),
            });
        }
    ...
    /// <summary>
    /// How many RaceMenu body sliders the preset actually shapes — the "bodyMorphs" block, which
    /// is where OBody/BodySlide morphs live. A plugin has nowhere to store these: an NPC record
    /// carries head data, not a body shape, so they are silently lost on export unless the build
    /// says otherwise. Zero-valued sliders are excluded because RaceMenu writes the whole slider
    /// set whether it was touched or not (measured on a reference preset: 119 present, 86 shaped).
    /// </summary>
    public int ReadJslotBodyMorphCount(string jslotPath)
    {
        try
        {
            using var stream = File.OpenRead(jslotPath);
            using var document = JsonDocument.Parse(stream);
            if (!document.RootElement.TryGetProperty("bodyMorphs", out var morphs)
                || morphs.ValueKind != JsonValueKind.Array)
                return 0;

            return morphs.EnumerateArray().Count(morph =>
                morph.TryGetProperty("keys", out var keys)
                && keys.ValueKind == JsonValueKind.Array
                && keys.EnumerateArray().Any(key =>
                    key.TryGetProperty("value", out var value)
                    && value.TryGetSingle(out var amount)
                    && amount != 0f));
        }
        catch (Exception ex)
        {
    ...
    private ParsedJslot? ReadJslot(string jslotPath)
    {
        try
        {
            ...
            return new ParsedJslot(
                ...
                {
                    HeadParts = headParts,
                    UnresolvedHeadParts = unresolvedHeadParts,
                    Weight = weight,
    ...
    /// <summary>
    /// Shape count for black-face / empty-export detection.
    /// Returns the NiShape count when the NIF loads; 0 when it loads but is empty;
    /// -1 when nifly cannot open the file (do not treat as "no shapes" without a successful load).
    /// </summary>
    private static int CountShapes(string nifPath)
    {
        try
        {
            using var nif = new NifHeadFile();
            nif.Load(nifPath);
            return nif.Shapes().Count;
        }
        catch (Exception)
        {
            return -1;
