...
The fallback behind that warning was wrong. On a real head (MaleHeadArgonian, MASSEESL.esp), 3.7.2 rewrote shader slot 7 and left slot 6 pointing at the old facetint. Slot 7 on that mesh is ArgonianMaleHead_s.dds. Slot 6 is the tint. A RaceMenu export (AAA_GirlHeads) stores the CharGen DDS on slot 6 and femalehead_s.dds on slot 7. The preset faceTextures block on this machine agrees: index 7 is FemaleHead_S.dds.

3.7.3 rewrites the slot that actually references a CharGen or facetint path. If nothing does, it claims slot 6 only when that slot is empty or already a tint, and only on a head shape (name, or diffuse file name). Otherwise it still refuses, in one warning that includes the Ctrl+F4 steps and the report path.

524 Release tests passed (8 new). Live swap of the two real heads above, using the 3.7.3 build, wrote the new tint path on slot 6 and left slot 7 as the specular map. No in-game check.
...
Owner: Hermes / Union Alpha. Parent: released 3.7.0; 240 files copied with zero SHA-256 mismatches (bin/obj/dist excluded).
Baseline: 488 passed, 1 failed (batch expected 2, got 1); isolated rerun passed. Cause unconfirmed.
Confirmed investigation: MO2 upstream profile.cpp writes highest-priority first; parser currently leaves it reversed for last-wins consumers.
Plan: regression tests and minimal fixes; full Release and CLI/UI smoke checks; verified local package. No publication or game deployment.
GitHub main has two newer documentation-only commits; preserve them before any publication.

# Parent state (historical)
