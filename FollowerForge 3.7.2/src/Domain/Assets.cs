...
public sealed record CharGenExport
{
    ...
    public IReadOnlyList<string> QualityNotes
    {
        get
        {
            ...
            // RaceMenu Export Head bakes the live face (including slider pose) into the NIF.
            // The matching jslot often still has empty sculpt.data (FaceForge / slider workflows).
            // Only warn about "slider-only" when we do NOT have a verified multi-shape head mesh.
            if (PresetAppearance is { SliderCount: >= 10, SculptedVertices: 0 }
                && HeadShapeCount <= 0)
                notes.Add(
                    $"shape is {PresetAppearance.SliderCount} RaceMenu sliders with no sculpt deltas and no verified head mesh — "
                    + "in RaceMenu's Sculpt tab use [F5] Export Head (close the console first, save it under the preset's name) "
                    + "so the .nif lands in Data\\SKSE\\Plugins\\CharGen with the geometry baked in");
            ...
            if (PresetAppearance is { UnresolvedHeadParts: > 0 } unresolved)
                notes.Add(
                    $"{unresolved.UnresolvedHeadParts} head part(s) the preset names could not be resolved to a record — "
                    + "her hair/eyes/brows may be missing in game. The plugin that provides them is missing from the preset's "
                    + "mods list; enable it and re-save the preset with RaceMenu.");
            return notes;
        }
    }
}
