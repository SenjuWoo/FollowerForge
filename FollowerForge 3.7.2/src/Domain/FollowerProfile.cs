...
/// <summary>
/// A reference to one record. <paramref name="Count"/> is only meaningful for inventory items
/// (arrows come in stacks); every other list ignores it and leaves it at 1. Defaulted so older
/// profile JSON without the field still loads.
/// </summary>
public sealed record RecordRef(string FormKey, int Count = 1)
{
    public override string ToString() => Count > 1 ? $"{FormKey} x{Count}" : FormKey;
}
...
public sealed record PlacementSpec
{
    ...
    /// <summary>FormKey of the CELL (interior) or worldspace persistent cell to place the ACHR in.</summary>
    public RecordRef? Cell { get; init; }
    /// <summary>
    /// Exterior only: FormKey of the WORLDSPACE that owns <see cref="Cell"/> (the worldspace
    /// top/persistent cell). Required for manual exterior placement outside WhiterunWorld.
    /// Interiors leave this null.
    /// </summary>
    public RecordRef? Worldspace { get; init; }
...
public sealed record AppearanceSpec
{
    ...
    /// <summary>
    /// TXST the preset's face was built on (NPC record FTST). Left null the follower inherits her
    /// race's default complexion, which does not have to match the exported head — the usual
    /// cause of a skin-tone seam at the neck.
    /// </summary>
    public RecordRef? HeadTextureSet { get; init; }

    /// <summary>
    /// Head-part entries the preset referenced that could not be turned into a FormKey (no
    /// readable formIdentifier and no mods entry covering the raw formId's index). Such a part
    /// is silently absent from the built NPC — a dropped hair entry means a bald follower with
    /// every screen reporting success. Surfaced as a warning rather than dropped in silence.
    /// </summary>
    public int UnresolvedHeadParts { get; init; }

    /// <summary>
    /// Non-zero RaceMenu slider values in the preset (CME_/EFM_/SPG_). These move the head's
    /// nodes on a live actor and are not vertex data, so they reach a follower only in so far as
    /// they were baked into the exported head mesh.
    /// </summary>
    public int SliderCount { get; init; }

    /// <summary>
    /// Vertices the preset sculpts by hand. Sculpt is real geometry and always survives into an
    /// exported head; a preset with none carries its whole shape in sliders.
    /// </summary>
    public int SculptedVertices { get; init; }
}
...
public sealed record FollowerProfile
{
    ...
    /// <summary>
    /// Arrows and bolts, carried in inventory with their stack count. Deliberately NOT added to
    /// her outfit: checked against Faendal, the game's own archer follower, who carries Steel
    /// Arrow (01397F) in Items with a clothes-only outfit. No vanilla outfit references ammo.
    /// </summary>
    public IReadOnlyList<RecordRef> Ammo { get; init; } = [];
