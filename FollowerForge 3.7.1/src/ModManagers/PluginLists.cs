...
public static class PluginLists
{
    ...
    /// <summary>
    /// Parses MO2 modlist.txt. Lines starting with '+' are enabled; '-' are disabled.
    /// Disk order is highest priority first. Normalize to last-wins order for consumers.
    /// Returns only enabled mod folder names, lowest→highest.
    /// </summary>
    public static IReadOnlyList<string> ParseMo2ModList(string path)
    {
        var result = new List<string>();
        foreach (var raw in File.ReadLines(path))
        {
            ...
            if (name.Length == 0) continue;
            // MO2 separators / empty markers are not real folders.
            if (name.StartsWith("----------", StringComparison.Ordinal)) continue;
            result.Add(name);
        }
        // MO2 Profile::doWriteModlist writes reverse priority (highest first).
        // Consumers use last-wins order, so normalize at the parser boundary.
        result.Reverse();
        return result;
    }
}
